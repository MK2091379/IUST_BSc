...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [(GHOST0, PAC), (GHOST1, PAC), (GHOST0, OBS0), (GHOST1, OBS1)]
    variableDomainsDict = {}

    "*** YOUR CODE HERE ***"
    # Populate the variableDomainsDict for ghosts and pacman
    ...
    max_distance = max(manhattanDistance((0, 0), (x, y)) for x in range(X_RANGE) for y in range(Y_RANGE))

    # Populate the variableDomainsDict for observations
    obs_domain = list(range(max_distance + MAX_NOISE + 1))  # Adding 1 because range is exclusive on the upper bound
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        # If the total is 0, we do nothing
        if total == 0:
            return
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Get the total sum of the values in the distribution
        total = self.total()
        
        # Generate a random number between 0 and total
        rand_value = random.random() * total
        
        # Iterate through the distribution and return the appropriate key
        cumulative = 0
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            # If the ghost is in jail and the noisyDistance is None, return 1.0 (this is the correct observation)
            if noisyDistance is None:
                return 1.0
            # If the ghost is in jail and the noisyDistance is not None, return 0.0 (invalid observation)
            else:
                return 0.0

        # If the noisyDistance is None but the ghost is not in jail, return 0.0 (invalid observation)
        if noisyDistance is None:
            return 0.0

        # Compute the true distance using Manhattan distance
        trueDistance = manhattanDistance(pacmanPosition, ghostPosition)

        # Return the probability of observing noisyDistance given the true distance
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Get the ghost's jail position
        jailPosition = self.getJailPosition()

        # Iterate over all possible ghost positions, including the jail position
        for pos in self.allPositions:
            # Get the current belief for this ghost position
            currentBelief = self.beliefs[pos]
            
            # Get the observation probability for this position
            observationProb = self.getObservationProb(observation, pacmanPosition, pos, jailPosition)
            
            # Update the belief by multiplying with the observation probability
            self.beliefs[pos] = currentBelief * observationProb
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Create a new distribution to hold the updated beliefs
        newBeliefs = DiscreteDistribution()

        # Iterate over all old positions (possible positions at time t)
        for oldPos in self.allPositions:
            # Get the distribution over new positions at time t + 1
            newPosDist = self.getPositionDistribution(gameState, oldPos)

            # Update the beliefs by adding in the probability of transitioning
            # from oldPos to each new position
            for newPos, prob in newPosDist.items():
                newBeliefs[newPos] += self.beliefs[oldPos] * prob

        # Update self.beliefs with the new beliefs after time elapse
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        # Ensure particles are evenly distributed over legal positions
        for i in range(self.numParticles):
            position = self.legalPositions[i % numLegalPositions]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Get Pacman's position and the jail position
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()

        # Initialize a new distribution for the weights
        weightDistribution = DiscreteDistribution()

        # Loop over each particle and compute its weight based on the observation
        for particle in self.particles:
    ...
    def elapseTime(self, gameState):
        ...
        # For each particle (representing a possible ghost position)
        for oldPos in self.particles:
            # Get the distribution over new positions given the old position
            newPosDist = self.getPositionDistribution(gameState, oldPos)

            # Resample a new position from this distribution
            newParticle = newPosDist.sample()

            # Add the resampled new position to the new particles list
            newParticles.append(newParticle)

        # Update particles with the new sampled positions
