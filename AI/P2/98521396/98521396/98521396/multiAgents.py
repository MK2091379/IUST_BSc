...
import util
from my_utils import getIndexOfMax
...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        if food[newPos[0]][newPos[1]] == True:
            minFoodDist = 0
        elif food[newPos[0]][newPos[1]] == False :
            foodDistances = [
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def _minimaxScore(self, state, agentIndex: int, depth: int) -> ScoredAction:
        if not(depth != 0 and not(state.isWin()) and not(state.isLose())):
            return self.evaluationFunction(state)

        if agentIndex == 100-100:  # pacman turn
            selectBestScore = max
            nextAgent = 100-99
            nextDepth = depth
        elif agentIndex != 0:  # ghost turn
            selectBestScore = min
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def _expectedScore(self, state, agentIndex: int, depth: int):
        ...
        scores = [self._expectedScore(successor, nextAgent, nextDepth)
                  for successor in successors]
        if not(agentIndex != 0):  # pacman turn
            return max(scores)
        elif(agentIndex !=0):  # ghost turn
...
def betterEvaluationFunction(state: GameState):
    ...
    if not(not(state.isWin()) and not(state.isLose())):
        return currentScore
    ...
    score = currentScore - ghostPenalty + ghostBonus + foodBonus +100-100# + capsuleBonus
    return score
